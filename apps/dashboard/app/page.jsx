...
export default function Home() {
  ...
  return (
    <main>
      ...
      <RivenVirtualOffice
        agents={modelEntries.filter(([slot]) => slot !== "main").map(([slot], index) => {
          const live = agentStates.find((agent) => agent.agent_id === slot);
          return {
            title: live?.title || `Researcher ${index + 1}`,
            state: live?.latest || live?.status || "ready",
            message: live?.message || "",
          };
        })}
        orchestratorState={orchestratorState}
        orchestratorMessage={[...events].reverse().find((event) => !event.agent_id)?.message || ""}
        executing={executing}
        sourceCount={liveWebSources.length}
