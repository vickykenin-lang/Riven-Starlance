...
function drawRoleVisualization(ctx, role, accent, active) {
  ...
  if (mode.includes("code")) {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(135, 256); ctx.lineTo(135, 348); ctx.moveTo(89, 302); ctx.lineTo(181, 302); ctx.stroke();
    ...
    ctx.stroke();
    pts.forEach(([x,y]) => { ctx.fillStyle = accent; ctx.beginPath(); ctx.arc(x,y,7,0,Math.PI*2); ctx.fill(); });
    ...
    [[72,258,150,84],[248,258,150,84],[424,258,150,84],[600,258,96,84]].forEach(([x,y,w,h], i) => {
      ...
      ctx.fillStyle = g; ctx.globalAlpha = .6; ctx.fillRect(x,y,w,h); ctx.globalAlpha = 1;
    });
  } else if (mode.includes("planning") || mode.includes("strategy") || mode.includes("operations")) {
    const bars = [0.86,0.62,0.74,0.48,0.9];
    ...
    ctx.strokeStyle = accent; ctx.lineWidth = 5; ctx.beginPath();
    values.forEach((v,i) => { const x = 74 + i * 72; const y = 350 - v; i ? ctx.lineTo(x,y) : ctx.moveTo(x,y); });
    ctx.stroke();
    values.forEach((v,i) => { ctx.fillStyle = accent; ctx.beginPath(); ctx.arc(74 + i*72,350-v,6,0,Math.PI*2); ctx.fill(); });
  } else {
    for (let i = 0; i < 7; i += 1) {
      const h = 28 + ((i * 31) % 76);
...
function makePanelTexture(station) {
  ...
  drawWrapped(ctx, station.task || station.role || "Standing by", 46, 150, 660, 34, 2);
...
function StationPanel({ station, position, onSelect }) {
  ...
  return (
    <group ref={panelRef} position={[position[0], 3.05, position[2]]} onClick={(event) => { event.stopPropagation(); onSelect(station.id); }}>
      <mesh>
      ...
      {!station.reserve && station.kind === "working" && (
        <pointLight color={COLORS[station.accent] || COLORS.cyan} intensity={1.25} distance={4.5} position={[0, 0, .5]} />
      )}
    </group>
  );
}
  ...
  return (
    <group ref={ref} position={[0, 0.62, 0.78]}>
      ...
      <mesh position={[0, .62, 0]} rotation={[0.12, 0, 0]}>
        <cylinderGeometry args={[0.27, 0.37, .82, 16]} />
      ...
      <mesh position={[-.34, .7, -.18]} rotation={[0.4, 0, -.9]}><cylinderGeometry args={[.07,.08,.65,10]} /><meshStandardMaterial color="#4e6f88" /></mesh>
      <mesh position={[.34, .7, -.18]} rotation={[0.4, 0, .9]}><cylinderGeometry args={[.07,.08,.65,10]} /><meshStandardMaterial color="#4e6f88" /></mesh>
    </group>
  );
}
...
function Workstation({ station, position, rotation, onSelect }) {
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      <mesh position={[0, .42, .05]}>
        <boxGeometry args={[2.25, .12, 1.1]} />
        <meshStandardMaterial color="#071222" metalness={0.8} roughness={0.22} emissive={accent} emissiveIntensity={station.reserve ? .025 : .07} />
      </mesh>
      <mesh position={[0, .18, .74]}>
        <boxGeometry args={[.72, .72, .72]} />
        <meshStandardMaterial color="#09111d" metalness={.82} roughness={.24} />
      </mesh>
      <mesh position={[-.85,.15,.05]}><boxGeometry args={[.12,.55,.92]} /><meshStandardMaterial color="#0a1a2d" /></mesh>
      <mesh position={[.85,.15,.05]}><boxGeometry args={[.12,.55,.92]} /><meshStandardMaterial color="#0a1a2d" /></mesh>
      <AgentOperator station={station} />
      <mesh position={[0, .48, -.5]} rotation={[-.12, 0, 0]}>
        <boxGeometry args={[1.9, .92, .08]} />
        <meshStandardMaterial color="#06111e" emissive={accent} emissiveIntensity={station.kind === "working" ? .28 : .06} />
      </mesh>
    </group>
  );
}
...
function DataLink({ end, station }) {
  ...
  const curve = useMemo(() => new CatmullRomCurve3([
    new Vector3(0, .4, 0),
    new Vector3(end[0] * .42, 1.15, end[2] * .42),
    new Vector3(end[0], .35, end[2]),
  ]), [end]);
  const geometry = useMemo(() => new TubeGeometry(curve, 28, .016, 6, false), [curve]);
  useEffect(() => () => geometry.dispose(), [geometry]);

  useFrame(({ clock }) => {
    if (!pulse.current || station.reserve || station.kind !== "working") return;
    const t = (clock.getElapsedTime() * .24 + station.index * .071) % 1;
  ...
  return (
    <group>
      <mesh geometry={geometry}>
        <meshBasicMaterial color={accent} transparent opacity={station.kind === "working" ? .5 : .12} toneMapped={false} />
      </mesh>
...
function RivenHologram({ active }) {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (figure.current) figure.current.position.y = 2.5 + Math.sin(t * 1.5) * .08;
    if (ringA.current) ringA.current.rotation.z = t * .28;
    if (ringB.current) ringB.current.rotation.x = 1.2 + t * .22;
    if (ringC.current) ringC.current.rotation.y = t * -.2;
  });
  const glow = active ? 1.35 : .8;
  const material = <meshStandardMaterial color="#8cecff" emissive="#23cfff" emissiveIntensity={glow} transparent opacity={.72} metalness={.15} roughness={.2} />;

  return (
    <group>
      <mesh position={[0,.12,0]}><cylinderGeometry args={[2.15,2.35,.34,64]} /><meshStandardMaterial color="#07182b" metalness={.85} roughness={.2} emissive="#1a8fd4" emissiveIntensity={.25} /></mesh>
      <mesh position={[0,.34,0]}><torusGeometry args={[1.72,.055,10,80]} /><meshBasicMaterial color="#55e9ff" toneMapped={false} /></mesh>
      <group ref={figure} position={[0,2.5,0]}>
        <mesh position={[0,1.05,0]}><sphereGeometry args={[.34,24,24]} />{material}</mesh>
        <mesh position={[0,.22,0]}><cylinderGeometry args={[.34,.5,1.18,20]} />{material}</mesh>
        <mesh position={[-.58,.35,0]} rotation={[0,0,-.78]}><cylinderGeometry args={[.075,.09,1.2,12]} />{material}</mesh>
        <mesh position={[.58,.35,0]} rotation={[0,0,.78]}><cylinderGeometry args={[.075,.09,1.2,12]} />{material}</mesh>
        <mesh position={[-.24,-.7,0]} rotation={[0,0,.12]}><cylinderGeometry args={[.09,.1,1.25,12]} />{material}</mesh>
        <mesh position={[.24,-.7,0]} rotation={[0,0,-.12]}><cylinderGeometry args={[.09,.1,1.25,12]} />{material}</mesh>
        <pointLight color="#2edcff" intensity={active ? 4 : 2} distance={8} />
      </group>
      <mesh ref={ringA} position={[0,2.7,0]} rotation={[1.35,0,.2]}><torusGeometry args={[1.25,.018,8,96]} /><meshBasicMaterial color="#3ee8ff" transparent opacity=.75 toneMapped={false} /></mesh>
      <mesh ref={ringB} position={[0,2.7,0]} rotation={[.2,.3,0]}><torusGeometry args={[1.65,.015,8,96]} /><meshBasicMaterial color="#9172ff" transparent opacity=.55 toneMapped={false} /></mesh>
      <mesh ref={ringC} position={[0,2.7,0]} rotation={[.6,.8,.4]}><torusGeometry args={[2.0,.012,8,96]} /><meshBasicMaterial color="#4fffb2" transparent opacity=.35 toneMapped={false} /></mesh>
    </group>
  );
}
  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2,0,0]} position={[0,-.02,0]}>
        ...
        <meshStandardMaterial color="#020916" metalness={.68} roughness={.32} />
      </mesh>
      {[3.2,6.8,10.2,12.2].map((radius) => <mesh key={radius} rotation={[Math.PI / 2,0,0]} position={[0,.015,0]}><torusGeometry args={[radius,.025,8,96]} /><meshBasicMaterial color={radius < 7 ? "#1ccfff" : "#184a6b"} transparent opacity={radius < 7 ? .32 : .2} toneMapped={false} /></mesh>)}
      {pillars.map(([x,z], i) => <mesh key={i} position={[x,2.3,z]}><boxGeometry args={[.18,4.6,.18]} /><meshStandardMaterial color="#09213a" emissive={i % 3 === 0 ? "#1b77a8" : "#04111d"} emissiveIntensity=.35 metalness=.8 roughness=.2 /></mesh>)}
      <mesh position={[0,5.1,-10.8]}><boxGeometry args={[20,.08,.08]} /><meshBasicMaterial color="#2f95c8" transparent opacity=.35 /></mesh>
    </group>
  );
}
...
function StarField() {
  ...
  const geometry = useMemo(() => {
    ...
    g.setAttribute("position", new (require("three").BufferAttribute)(positions, 3));
    return g;
  }, [positions]);
  useEffect(() => () => geometry.dispose(), [geometry]);
  return <points geometry={geometry}><pointsMaterial color="#72cfff" size=.055 transparent opacity=.7 sizeAttenuation /></points>;
}
...
function CommandRoom({ stations, orchestratorActive, selectedId, onSelect }) {
  ...
  return (
    <>
      ...
      <ambientLight intensity=.38 color="#77bde8" />
...
export default function RivenCommandRoomScene({ stations, orchestratorActive, selectedId, onSelect }) {
  return (
    <div className="room3d-canvas-wrap">
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 11.6, 22.5], fov: 43, near: .1, far: 70 }} gl={{ antialias: true, alpha: false, powerPreference: "high-performance" }}>
